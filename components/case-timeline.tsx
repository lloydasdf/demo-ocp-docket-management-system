...
function normalizedCourtStatus(value: string) {
  return value.trim().replace(/\s+/g, " ").toLocaleLowerCase();
}

function mergeCourtStatusRows(...groups: CourtStatusRow[][]) {
  const statuses = new Map<string, CourtStatusRow>();

  for (const row of groups.flat()) {
    const courtStatus = row.court_status.trim();
    const statusDate = row.status_date.trim();
    if (!courtStatus) continue;

    const key = normalizedCourtStatus(courtStatus);
    const existing = statuses.get(key);
    if (!existing || (!existing.status_date && statusDate)) {
      statuses.set(key, { court_status: courtStatus, status_date: statusDate });
    }
  }
...
function snapshotCourtStatusRows(details: Record<string, unknown>): CourtStatusRow[] {
  if (Array.isArray(details.court_statuses)) {
    return details.court_statuses.map((row) => {
      const item = row as Record<string, unknown>;
      return {
        court_status: String(item.court_status ?? ""),
        status_date: toDateInputValue(String(item.status_date ?? "")),
      };
    });
  }

  return details.court_status ? [{ court_status: String(details.court_status), status_date: "" }] : [];
}
      ...
      <div className="space-y-2">
        {rows.map((row, index) => (
          <div key={index} className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
            <Input placeholder="Detail" value={row.detail} onChange={(e) => onChange(rows.map((item, itemIndex) => itemIndex === index ? { ...item, detail: e.target.value } : item))} />
            <Input placeholder="Value" value={row.value} onChange={(e) => onChange(rows.map((item, itemIndex) => itemIndex === index ? { ...item, value: e.target.value } : item))} />
            <Button type="button" variant="outline" size="sm" onClick={() => onChange(rows.filter((_, itemIndex) => itemIndex !== index))}>Remove</Button>
          </div>
        ))}
      </div>
      <Button type="button" variant="outline" size="sm" onClick={() => onChange([...rows, { detail: "", value: "" }])}>Add Detail</Button>
    </div>
  );
}
...
function courtFilingEventDetails(event: CaseTimelineEventRecord, court: CaseCourtRecord | null) {
  ...
  const courtStatuses = mergeCourtStatusRows(
    snapshotCourtStatusRows(details),
    court?.court_status ? [{ court_status: court.court_status, status_date: "" }] : [],
  ).map((row) => `${row.court_status} — ${formatDate(row.status_date)}`).join("; ");
...
function shouldShowCourtSourceDetails(event: CaseTimelineEventRecord) {
  // COURT_FILING events merge any linked case_courts values into their unified
  // Court Filing card. The standalone source card remains a legacy fallback.
...
export function CaseTimeline({
  ...
  const [hasInitializedCourtFilingDecisions, setHasInitializedCourtFilingDecisions] = useState(false);
  ...
  const [addForm, setAddForm] = useState({ eventTypeCode: "", eventDate: initialManilaDateTime.date, eventTime: "", title: "", description: "", prosecutorId: "", staffId: "", reason: "", recommendationCode: "", caseResolutionId: null as number | null, chargesForFiling: [emptyResolutionCharge()], chargesForDismissal: [emptyResolutionCharge()], approvalActions: [emptyApprovalAction()], courtFilingDecisionIds: [] as string[], courtFilingCharges: {} as Record<string, string>, courtId: null as number | null, courtName: "", courtBranch: "", chargeFiled: "", informationCount: "", criminalCaseNumbers: [{ criminal_case_no: "" }] as CriminalCaseNumberRow[], motionTitle: "", caseMotionId: "", motionResolutionId: "", filedByCode: "", assignedProsecutorId: "", motionRecommendationId: "", motionDecisionStep: 1, motionApprovalDecisionId: "", motionApprovedByProsecutorId: "", motionUpdateCaseStatus: "", motionSelectedCaseStatusId: "", motionSelectedCaseStageId: "", motionDetails: [] as MotionDetailRow[], petitionStatus: "", petitionUpdateStep: 1, petitionUpdatePetitionId: "", petitionUpdateCaseStatus: "", petitionSelectedCaseStatusId: "", petitionSelectedCaseStageId: "", petitionDetails: [] as MotionDetailRow[], customStep: 1, customUpdateCaseStatus: "", customSelectedCaseStatusId: "", customSelectedCaseStageId: "", customDetails: [] as MotionDetailRow[] });
  ...
  const [prosecutorSelectionTarget, setProsecutorSelectionTarget] = useState<"add" | "edit">("add");

  const openProsecutorDialog = (target: "add" | "edit" = "add") => {
    setProsecutorForm({ fiscalCode: "" });
    setProsecutorFormError(null);
    setProsecutorSelectionTarget(target);
  ...
  const handleCreateProsecutor = async () => {
    ...
    try {
      ...
      if (prosecutorSelectionTarget === "edit") setEditForm((form) => ({ ...form, prosecutorId: String(prosecutor.id) }));
      else setAddForm((form) => ({ ...form, prosecutorId: String(prosecutor.id) }));
  ...
  const openAddDialog = async () => {
    ...
    setHasInitializedCourtFilingDecisions(false);
    ...
    setAddForm({ eventTypeCode: addableEventTypes(eventTypes)[0]?.code ?? "", eventDate: currentManilaDateTime.date, eventTime: "", title: "", description: "", prosecutorId: "", staffId: "", reason: "", recommendationCode: "", caseResolutionId: null, chargesForFiling: [emptyResolutionCharge()], chargesForDismissal: [emptyResolutionCharge()], approvalActions: [emptyApprovalAction()], courtFilingDecisionIds: [] as string[], courtFilingCharges: {} as Record<string, string>, courtId: null as number | null, courtName: "", courtBranch: "", chargeFiled: "", informationCount: "", criminalCaseNumbers: [{ criminal_case_no: "" }] as CriminalCaseNumberRow[], motionTitle: "", caseMotionId: "", motionResolutionId: "", filedByCode: "", assignedProsecutorId: "", motionRecommendationId: "", motionDecisionStep: 1, motionApprovalDecisionId: "", motionApprovedByProsecutorId: "", motionUpdateCaseStatus: "", motionSelectedCaseStatusId: "", motionSelectedCaseStageId: "", motionDetails: [] as MotionDetailRow[], petitionStatus: "", petitionUpdateStep: 1, petitionUpdatePetitionId: "", petitionUpdateCaseStatus: "", petitionSelectedCaseStatusId: "", petitionSelectedCaseStageId: "", petitionDetails: [] as MotionDetailRow[], customStep: 1, customUpdateCaseStatus: "", customSelectedCaseStatusId: "", customSelectedCaseStageId: "", customDetails: [] as MotionDetailRow[] });
    ...
    const motionResolutionCandidatesResult = await getMotionResolutionApprovalCandidates(caseId);
    if (!motionResolutionCandidatesResult.error) setMotionResolutionApprovalCandidates(motionResolutionCandidatesResult.data);
  ...
  const handleAddSave = async () => {
    ...
    if (isCourtFiling && (!addForm.courtName.trim() || (addForm.courtFilingDecisionIds.length ? addForm.courtFilingDecisionIds.some((id) => !addForm.courtFilingCharges[id]?.trim()) : !addForm.chargeFiled.trim()))) return;
    if (isCourtStatusUpdate && courtStatusStep !== 3) return;
      ...
      : isReassignment
        ...
        : isResolved
          ...
          : isDecisionApproved
            ...
            : isCourtFiling
              ? await recordCourtFilingEvents({
                caseId,
                filings: addForm.courtFilingDecisionIds.length ? addForm.courtFilingDecisionIds.map((id) => ({ caseResolutionApprovalActionId: Number(id), chargeFiled: addForm.courtFilingCharges[id] })) : [{ caseResolutionApprovalActionId: null, chargeFiled: addForm.chargeFiled }],
                ...
                criminalCaseNumbers: addForm.criminalCaseNumbers.map((row) => row.criminal_case_no.trim()).filter(Boolean),
                remarks: addForm.description,
              })
              : isCourtStatusUpdate
                ? await updateCourtFilingStatusDetails({
                  caseId,
                  courtFilingId: courtStatusForm.courtFilingId ? Number(courtStatusForm.courtFilingId) : null,
                ...
                : isMotionResolved
                  ? await recordMotionResolvedEvent({
                    caseId,
                    caseMotionId: addForm.caseMotionId ? Number(addForm.caseMotionId) : null,
                    motionTitle: activeUnresolvedMotions.length === 0 ? addForm.motionTitle : null,
                  ...
                  : isMotionDecisionApproved
                    ? await recordMotionDecisionApprovedEvent({
                      caseId,
                      motionTitle: addForm.motionTitle,
                      caseMotionResolutionId: addForm.motionResolutionId ? Number(addForm.motionResolutionId) : null,
                    ...
                    : addForm.eventTypeCode === "PETITION_FOR_REVIEW"
                      ...
                      : addForm.eventTypeCode === "PETITION_FOR_REVIEW_UPDATE"
                        ? await recordPetitionForReviewUpdate({
                          caseId,
                          petitionForReviewId: addForm.petitionUpdatePetitionId ? Number(addForm.petitionUpdatePetitionId) : null,
                          petitionStatus: addForm.petitionStatus,
    ...
    const recordedWithoutPrerequisite =
      ...
      (isCourtFiling && addForm.courtFilingDecisionIds.length === 0);
  ...
  const openEditDialog = async (event: CaseTimelineEventRecord) => {
    ...
    const sourceAssignment = assignments.find((assignment) => Number(assignment.id) === Number(event.source_id));
    ...
    setEditForm({
      ...
      description: isCourtFilingEvent(event)
        ? event.description ?? ""
        : sourceMotion?.remarks ?? sourcePetition?.remarks ?? stringDetail(details.remarks) ?? event.description ?? "",
      remarks: courtFiling?.court_status_update_remarks ?? courtFiling?.remarks ?? legacyCourt?.court_remarks ?? stringDetail(details.remarks) ?? "",
      reason: stringDetail(details.reason) ?? "",
      editReason: "",
      prosecutorId: String(sourceAssignment?.prosecutor_id ?? details.new_prosecutor_id ?? ""),
      staffId: String(sourceAssignment?.staff_id ?? details.staff_id ?? ""),
      assignedProsecutorId: String(sourceMotion?.assigned_prosecutor_id ?? details.assigned_prosecutor_id ?? details.approved_by_prosecutor_id ?? details.approved_by_id ?? ""),
      ...
      courtStatuses: mergeCourtStatusRows(
        courtFiling?.court_statuses.map((row) => ({ court_status: row.court_status, status_date: toDateInputValue(row.status_date) })) ?? [],
        snapshotCourtStatusRows(details),
        legacyCourt?.court_status ? [{ court_status: legacyCourt.court_status, status_date: "" }] : [],
      ),
  ...
  const handleEditSave = async () => {
    ...
    const result = code === "CASE_ASSIGNMENT" || code === "CASE_REASSIGNMENT"
      ...
      values: {
        ...
        remarks: editForm.description,
        ...(["CASE_ASSIGNMENT", "CASE_REASSIGNMENT", "CASE_RESOLVED", "PETITION_FOR_REVIEW"].includes(code) ? {} : { prosecutor_id: editForm.prosecutorId ? Number(editForm.prosecutorId) : null }),
        ...(["CASE_ASSIGNMENT", "CASE_REASSIGNMENT"].includes(code) ? {} : { staff_id: editForm.staffId ? Number(editForm.staffId) : null }),
        ...(["CASE_ASSIGNMENT", "CASE_REASSIGNMENT", "PETITION_FOR_REVIEW"].includes(code) ? {} : { assigned_prosecutor_id: editForm.assignedProsecutorId ? Number(editForm.assignedProsecutorId) : null }),
        ...(["PETITION_FOR_REVIEW"].includes(code) ? {} : { approved_by_prosecutor_id: editForm.approvedByProsecutorId ? Number(editForm.approvedByProsecutorId) : null }),
  ...
  const activeUnresolvedMotions = motions.filter((motion) => !motion.is_voided && !motion.active_motion_resolution_id);
  const isAddingMotionDecisionApproved = addForm.eventTypeCode === "MOTION_DECISION_APPROVED";
  const selectedMotionResolutionApprovalCandidate = motionResolutionApprovalCandidates.find((candidate) => Number(candidate.id) === Number(addForm.motionResolutionId)) ?? null;
  ...
  const openAddCourtDialog = () => {
    setCourtCreateForm({ name: addForm.courtName.trim(), error: "" });
  ...
  const handleAddCourtSave = async () => {
    ...
    const result = await addCourt({ name });
    setIsSaving(false);
    if (result.error) {
      setCourtCreateForm((form) => ({ ...form, error: result.error.message }));
      return;
    }
    const createdCourt: CourtReferenceRecord = { id: result.data, code: name.toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_+|_+$/g, "") || "COURT", name, court_type: null, is_active: true };
  ...
  useEffect(() => {
    if (addForm.eventTypeCode !== "COURT_FILING" || courtFilingDecisions.length === 0 || hasInitializedCourtFilingDecisions) return;
    setAddForm((form) => ({
      ...form,
      courtFilingDecisionIds: courtFilingDecisions.map((decision) => String(decision.id)),
      courtFilingCharges: Object.fromEntries(courtFilingDecisions.map((decision) => [String(decision.id), decision.charge_text])),
    }));
    setHasInitializedCourtFilingDecisions(true);
  }, [addForm.eventTypeCode, courtFilingDecisions, hasInitializedCourtFilingDecisions]);
        ...
        <DialogContent className="grid max-h-[calc(100vh-2rem)] grid-rows-[auto_minmax(0,1fr)_auto] gap-3 p-5 sm:max-w-xl">
          ...
          <div className="min-h-0 space-y-3 overflow-y-auto pr-1">
            ...
            {isAddingAssignmentLike ? (
              <>
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Button type="button" variant="outline" size="sm" onClick={() => openProsecutorDialog("add")}>+ Add prosecutor</Button>
                  </div>
              ...
              <>
                {courtFilingDecisions.length > 0 ? <div className="space-y-2"><div className="flex items-center justify-between"><Label>Approved Filing Decisions</Label><span className="text-xs text-muted-foreground">Selected {addForm.courtFilingDecisionIds.length} of {courtFilingDecisions.length}</span></div><div className="space-y-2 rounded-md border p-3">{courtFilingDecisions.map((decision) => { const id = String(decision.id); const checked = addForm.courtFilingDecisionIds.includes(id); return <label key={decision.id} className="flex items-start gap-2 text-sm"><Checkbox checked={checked} onCheckedChange={(value) => setAddForm((form) => ({ ...form, courtFilingDecisionIds: value === true ? [...form.courtFilingDecisionIds, id] : form.courtFilingDecisionIds.filter((item) => item !== id), courtFilingCharges: { ...form.courtFilingCharges, [id]: form.courtFilingCharges[id] ?? decision.charge_text } }))} /><span>{decision.charge_text} • Approved {formatDate(decision.date_approved)}</span></label>; })}</div></div> : null}
                ...
                {addForm.courtFilingDecisionIds.length ? <div className="space-y-3">{addForm.courtFilingDecisionIds.map((id, index) => <div key={id} className="space-y-1"><Label htmlFor={`add-charge-filed-${id}`}>Charge Filed {addForm.courtFilingDecisionIds.length > 1 ? index + 1 : ""}</Label><Input id={`add-charge-filed-${id}`} value={addForm.courtFilingCharges[id] ?? ""} onChange={(e) => setAddForm((form) => ({ ...form, courtFilingCharges: { ...form.courtFilingCharges, [id]: e.target.value } }))} /></div>)}</div> : <div className="space-y-2"><Label htmlFor="add-charge-filed">Charge Filed</Label><Input id="add-charge-filed" value={addForm.chargeFiled} onChange={(e) => setAddForm((form) => ({ ...form, chargeFiled: e.target.value }))} /></div>}
                ...
                <div className="grid gap-3 sm:grid-cols-2"><div className="space-y-1"><Label htmlFor="add-information-count">Information Count</Label><Input id="add-information-count" type="number" min="0" value={addForm.informationCount} onChange={(e) => setAddForm((form) => ({ ...form, informationCount: e.target.value }))} /></div><div className="space-y-1"><Label>Criminal Case No.</Label>{addForm.criminalCaseNumbers.map((row, index) => <div key={index} className="mt-2 flex gap-2"><Input aria-label={`Criminal Case Number ${index + 1}`} value={row.criminal_case_no} onChange={(e) => setAddForm((form) => ({ ...form, criminalCaseNumbers: form.criminalCaseNumbers.map((item, itemIndex) => itemIndex === index ? { criminal_case_no: e.target.value } : item) }))} /><Button type="button" size="sm" variant="outline" className="shrink-0" onClick={() => setAddForm((form) => ({ ...form, criminalCaseNumbers: index === 0 ? [...form.criminalCaseNumbers, { criminal_case_no: "" }] : form.criminalCaseNumbers.filter((_, itemIndex) => itemIndex !== index) }))}>{index === 0 ? "Add" : "Remove"}</Button></div>)}</div></div>
              ...
              <>
                {activeUnresolvedMotions.length > 0 ? (
                  <div className="space-y-2"><Label htmlFor="add-motion-resolution-motion">Motion (optional)</Label><select id="add-motion-resolution-motion" className="border-input h-9 w-full rounded-md border bg-transparent px-3 text-sm" value={addForm.caseMotionId} onChange={(e) => setAddForm((form) => ({ ...form, caseMotionId: e.target.value }))}><option value="">Select motion</option>{activeUnresolvedMotions.map((motion) => <option key={motion.id} value={motion.id}>{motion.motion_title ?? motion.motion_name ?? `Motion #${motion.id}`}</option>)}</select></div>
                ) : (
                  <div className="space-y-2"><Label htmlFor="add-motion-resolution-title">Motion Title (optional)</Label><Input id="add-motion-resolution-title" placeholder="Enter motion title" value={addForm.motionTitle} onChange={(e) => setAddForm((form) => ({ ...form, motionTitle: e.target.value }))} /></div>
                )}
              ...
              <>
                ...
                {addForm.motionDecisionStep === 1 ? <>
                  {motionResolutionApprovalCandidates.length > 0 ? (
                    <div className="space-y-2"><Label htmlFor="motion-approval-resolution">Motion Resolution</Label><select id="motion-approval-resolution" className="border-input h-9 w-full rounded-md border bg-transparent px-3 text-sm" value={addForm.motionResolutionId} onChange={(e) => setAddForm((form) => ({ ...form, motionResolutionId: e.target.value }))}><option value="">Select motion resolution</option>{motionResolutionApprovalCandidates.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.motion_title ?? `Motion resolution #${candidate.id}`}{candidate.recommendation_label ? ` — ${candidate.recommendation_label}` : ""}</option>)}</select></div>
                  ) : (
                    <div className="space-y-2"><Label htmlFor="motion-approval-title">Motion Title</Label><Input id="motion-approval-title" required value={addForm.motionTitle} onChange={(e) => setAddForm((form) => ({ ...form, motionTitle: e.target.value }))} /></div>
                  )}
                  ...
                  <div className="flex justify-end"><Button type="button" onClick={() => setAddForm((form) => ({ ...form, motionDecisionStep: 2 }))} disabled={(motionResolutionApprovalCandidates.length > 0 ? !addForm.motionResolutionId : !addForm.motionTitle.trim()) || !addForm.motionApprovalDecisionId}>Next</Button></div>
                </> : null}
      ...
      <Dialog open={isCourtDialogOpen} onOpenChange={setIsCourtDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Add a court to the active court list. Court code and metadata are generated automatically.</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={Boolean(editingEvent)} onOpenChange={(open) => !open && setEditingEvent(null)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            ...
            {editingEvent?.event_type_code !== "COURT_FILING" ? <DialogDescription>{editingEvent?.event_type_code === "CASE_ASSIGNMENT" || editingEvent?.event_type_code === "CASE_REASSIGNMENT" ? "Update the stored assignment details using the same fields as the original event." : "Correct stored timeline information without changing the event type, source record, or legal/workflow decision."}</DialogDescription> : null}
          </DialogHeader>
          <div className="space-y-4">
            {actionError ? <p className="rounded-md border border-destructive/30 bg-destructive/5 p-2 text-sm text-destructive">{actionError}</p> : null}
            {editingEvent?.event_type_code !== "COURT_FILING" && editingEvent?.event_type_code !== "CASE_ASSIGNMENT" && editingEvent?.event_type_code !== "CASE_REASSIGNMENT" ? <><EditNotice /><ReadOnlyActivityDetails event={editingEvent} /><div className="space-y-2"><p className="text-sm font-medium">Editable Corrections</p></div></> : null}
            {(() => {
              ...
              if (isAssignment) return <>
                <div className="space-y-2"><Label>Event Type</Label><select className="border-input h-9 w-full rounded-md border bg-muted px-3 text-sm" value={code} disabled><option value={code}>{code === "CASE_REASSIGNMENT" ? "Case Reassignment" : "Case Assignment"}</option></select></div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-2"><Label htmlFor="edit-assigned-prosecutor">{code === "CASE_REASSIGNMENT" ? "New Prosecutor" : "Assigned Prosecutor"}</Label><Button type="button" variant="outline" size="sm" onClick={() => openProsecutorDialog("edit")}>+ Add prosecutor</Button></div>
                  <select id="edit-assigned-prosecutor" className="border-input h-9 w-full rounded-md border bg-transparent px-3 text-sm" value={editForm.prosecutorId} onChange={(e) => setEditForm((form) => ({ ...form, prosecutorId: e.target.value }))}><option value="">Select prosecutor</option>{prosecutors.map((prosecutor) => <option key={prosecutor.id} value={prosecutor.id}>{prosecutor.short_name ?? prosecutor.full_name}</option>)}</select>
                  <p className="text-xs text-muted-foreground">Can’t find the prosecutor? Add them here and they will be selected automatically.</p>
                </div>
                <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="edit-assignment-date">{code === "CASE_REASSIGNMENT" ? "Reassignment Date" : "Assignment Date"}</Label><Input id="edit-assignment-date" type="date" value={editForm.eventDate} onChange={(e) => setEditForm((form) => ({ ...form, eventDate: e.target.value }))} /></div><div className="space-y-2"><Label htmlFor="edit-assignment-time">{code === "CASE_REASSIGNMENT" ? "Reassignment Time" : "Assignment Time"}</Label><Input id="edit-assignment-time" type="time" step="1" value={editForm.eventTime} onChange={(e) => setEditForm((form) => ({ ...form, eventTime: e.target.value }))} /></div></div>
                <div className="space-y-2"><Label htmlFor="edit-assigned-staff">{code === "CASE_REASSIGNMENT" ? "New Staff" : "Assigned Staff"}</Label><select id="edit-assigned-staff" className="border-input h-9 w-full rounded-md border bg-transparent px-3 text-sm" value={editForm.staffId} onChange={(e) => setEditForm((form) => ({ ...form, staffId: e.target.value }))}><option value="">No staff selected</option>{staffMembers.map((staff) => <option key={staff.id} value={staff.id}>{staff.short_name ?? staff.full_name}</option>)}</select></div>
                {code === "CASE_REASSIGNMENT" ? <div className="space-y-2"><Label htmlFor="edit-reassignment-reason">Reason</Label><Textarea id="edit-reassignment-reason" value={editForm.reason} onChange={(e) => setEditForm((form) => ({ ...form, reason: e.target.value }))} /></div> : null}
                <div className="space-y-2"><Label htmlFor="edit-assignment-remarks">Remarks</Label><Textarea id="edit-assignment-remarks" value={editForm.description} onChange={(e) => setEditForm((form) => ({ ...form, description: e.target.value }))} /></div>
              </>;
              return <>
                <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="space-y-2"><Label htmlFor="edit-description">{isCustomOrLegacy ? "Description / Remarks" : "Remarks"}</Label><Textarea id="edit-description" value={isCourtFiling ? editForm.remarks : editForm.description} onChange={(e) => setEditForm((form) => isCourtFiling ? { ...form, remarks: e.target.value } : { ...form, description: e.target.value })} /></div>
