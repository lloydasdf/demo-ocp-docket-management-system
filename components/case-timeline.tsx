...
function visibleEventDetails(event: CaseTimelineEventRecord) {
  ...
  const hiddenKeys = event.event_type_code === "CASE_RECEIVED"
    ...
    : event.event_type_code === "CASE_RESOLVED" || event.event_type_code === "CASE_DECISION_APPROVED"
      ? new Set(Object.keys(event.details_jsonb as Record<string, unknown>))
...
export function CaseTimeline({
  ...
  const handleAddSave = async () => {
    ...
    if (!isAssignment && !isReassignment && !isResolved && !isDecisionApproved && !addForm.title.trim()) return;
    setIsSaving(true);
    setActionError(null);
        ...
        <DialogContent className="grid max-h-[calc(100vh-2rem)] grid-rows-[auto_minmax(0,1fr)_auto] gap-3 p-5 sm:max-w-xl">
          ...
          <div className="min-h-0 space-y-3 overflow-y-auto pr-1">
            ...
            {isAddingAssignmentLike ? (
              ...
              <>
                {caseResolutions.length === 0 ? <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100">No active case resolution available to approve.</div> : null}
