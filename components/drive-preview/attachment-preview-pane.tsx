...
export function AttachmentPreviewPane({ file, blob, url, loading, onShare, onDownload, onStartEdit, checkEditStatus, cancelEditSession, onSaved, onError, onClose }: { file: PreviewFile; blob: Blob | null; url: string | null; loading: boolean; onShare: () => void; onDownload: () => void; onStartEdit: () => Promise<OnlyOfficeSession>; checkEditStatus: (sessionId: string) => Promise<{ status: string; last_error?: string | null }>; cancelEditSession: (sessionId: string) => Promise<void>; onSaved: () => void; onError: (message: string) => void; onClose: () => void }) {
  ...
  const extension = file.name.split('.').pop()?.toLowerCase(); const canEdit = extension === 'docx' || extension === 'xlsx' || extension === 'pptx';
  ...
  return <section ref={paneRef} className="flex h-full min-h-[72vh] flex-col overflow-hidden rounded-lg border bg-background shadow-xl"><PreviewToolbar file={file} zoom={zoom} rotation={rotation} canEdit={canEdit} editing={Boolean(editor) || launching} onEdit={() => void startEdit()} onZoom={setZoom} onRotate={() => setRotation((value) => (value + 90) % 360)} onShare={onShare} onDownload={onDownload} onFullscreen={() => void paneRef.current?.requestFullscreen()} onClose={onClose} /><div className="flex flex-1 items-center justify-center overflow-auto bg-muted/40 p-3">{editor ? <OnlyOfficeEditor session={editor} checkStatus={checkEditStatus} cancelSession={cancelEditSession} onSaved={editorSaved} onError={editorError} /> : loading || launching || !blob || !url ? <div className="flex items-center gap-2 text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin" />{launching ? 'Starting editor…' : 'Loading preview…'}</div> : <div style={{ transform: `scale(${zoom}) rotate(${rotation}deg)`, transformOrigin: "center" }} className="flex h-full w-full items-center justify-center transition-transform"><FilePreview file={file} blob={blob} url={url} onDownload={onDownload} /></div>}</div></section>;
}
