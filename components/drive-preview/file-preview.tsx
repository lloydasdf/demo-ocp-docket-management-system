...
export function FilePreview({ file, blob, url }: { file: PreviewFile; blob: Blob; url: string }) {
  ...
  if (mime === "application/pdf" || /\.pdf$/i.test(file.name)) return <PdfPreview url={url} name={file.name} />;
  ...
  if (mime.includes("wordprocessingml") || /\.docx$/i.test(file.name)) return <DocxPreview />;
