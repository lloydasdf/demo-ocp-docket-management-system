...
function ManageParticipantsDialog({ addressTypes, participantRoles, caseId, onOpenChange, onSaved, open, participants }: { addressTypes: RefOption[]; participantRoles: RefOption[]; caseId: number; onOpenChange: (open: boolean) => void; onSaved: () => Promise<void>; open: boolean; participants: CaseParticipantRecord[] }) {
  ...
  function startAdding(label: "Complainant" | "Respondent") {
    ...
    setFormData({ noMiddleName: false, roleId: String(role.id), remarks: "" });
  ...
  function participantButton(participant: CaseParticipantRecord) {
    return <button key={participant.id} type="button" className={`rounded-lg border p-3 text-left text-sm ${selectedId === participant.id ? "border-primary bg-primary/5" : "hover:bg-muted"}`} onClick={() => { setAdding(null); setSelectedId(participant.id); setMode(null); setEditingRecord(null); setPendingDelete(null); }}><span className="font-medium">{participantName(participant)}</span><span className="mt-1 block text-xs text-muted-foreground">{roleLabel(participant)}</span></button>;
  }
  ...
  const addParticipantEditor = adding ? <div className="space-y-4">
    <div><h3 className="text-lg font-semibold">Add {adding?.label}</h3><p className="text-sm text-muted-foreground">Step {adding?.step ?? 1} of 2: {adding?.step === 2 ? "Participant details" : "Participant type"}. After saving, use the same alias, address, and contact controls in Manage Participants.</p></div>
