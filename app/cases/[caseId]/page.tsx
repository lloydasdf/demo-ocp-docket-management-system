  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="flex max-h-[90vh] flex-col overflow-hidden sm:max-w-5xl"><DialogHeader className="shrink-0"><DialogTitle>Manage Participants</DialogTitle><DialogDescription>Select a participant, then edit main details, aliases, addresses, or contact information.</DialogDescription></DialogHeader>
    <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
      ...
      <div className="min-w-0 space-y-4 rounded-lg border p-4">{selected ? <><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h3 className="break-words text-lg font-semibold">{participantName(selected)}</h3><p className="text-sm text-muted-foreground">{roleLabel(selected)} • {selected.participant_kind ?? "PERSON"}</p>{legacyFullNameOnly ? <Badge variant="outline" className="mt-2">Legacy full-name only</Badge> : null}</div><Button type="button" size="sm" onClick={() => openEditor("main")}>Edit Main Details</Button></div>
        <div className="grid gap-3 sm:grid-cols-3"><div className="rounded-md border p-3"><div className="mb-2 flex items-center justify-between gap-2"><p className="font-medium">Aliases</p><Button type="button" size="sm" variant="outline" onClick={() => openEditor("alias")}>Add</Button></div>{aliases.length ? <div className="divide-y">{aliases.map((alias) => <div key={String(alias.id)} className="space-y-2 py-2 text-sm"><p className="break-words">{String(alias.alias_name ?? "")}</p><div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="ghost" onClick={() => openEditor("alias", alias)}>Edit</Button><Button type="button" size="sm" variant="ghost" className="text-destructive" onClick={() => { setMode("alias"); setEditingRecord(alias); void save("remove_alias", "Remove alias", alias); }}>Remove</Button></div></div>)}</div> : <p className="text-sm text-muted-foreground">No aliases.</p>}</div>
        <div className="rounded-md border p-3"><div className="mb-2 flex items-center justify-between gap-2"><p className="font-medium">Addresses</p><Button type="button" size="sm" variant="outline" onClick={() => openEditor("address")}>Add</Button></div>{addresses.length ? <div className="divide-y">{addresses.map((address) => <div key={String(address.id)} className="space-y-2 py-2"><p className="break-words text-sm">{formatAddress((address.addresses ?? {}) as never) || "Address"}</p><div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="ghost" onClick={() => openEditor("address", address)}>Edit</Button><Button type="button" size="sm" variant="ghost" className="text-destructive" onClick={() => { setMode("address"); setEditingRecord(address); void save("remove_address", "Remove address", address); }}>Remove</Button></div></div>)}</div> : <p className="text-sm text-muted-foreground">No addresses.</p>}</div>
        <div className="rounded-md border p-3"><div className="mb-2 flex items-center justify-between gap-2"><p className="font-medium">Contacts</p><Button type="button" size="sm" variant="outline" onClick={() => openEditor("contact")}>Add</Button></div>{contacts.length ? <div className="divide-y">{contacts.map((contact) => <div key={String(contact.participant_contact_information_id ?? contact.id)} className="space-y-2 py-2"><p className="text-xs font-medium uppercase text-muted-foreground">{String(contact.contact_type)}</p>{contactHref(contact as never) ? <a className="block break-all text-sm text-primary hover:underline" href={contactHref(contact as never)}>{String(contact.contact_value)}</a> : <p className="break-all text-sm">{String(contact.contact_value)}</p>}<div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="ghost" onClick={() => openEditor("contact", contact as never)}>Edit</Button><Button type="button" size="sm" variant="ghost" className="text-destructive" onClick={() => { setMode("contact"); setEditingRecord(contact as never); void save("remove_contact", "Remove contact", contact as never); }}>Remove</Button></div></div>)}</div> : <p className="text-sm text-muted-foreground">No contacts.</p>}</div></div>{renderEditor()}</> : <SectionEmpty>No participant selected.</SectionEmpty>}
      </div>
    </div>
