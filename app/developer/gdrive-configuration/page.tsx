...
type Status = { environment: EnvironmentRow[]; connection: { status: string; rootName?: string; message?: string; stage?: string; code?: string; suggestion?: string } };
      ...
      <div className="flex min-h-screen bg-background"><Sidebar />
        <main className="min-w-0 flex-1 overflow-y-auto p-6 lg:p-10">
          <div className="mx-auto max-w-5xl space-y-6">
            ...
            <Card><CardHeader><CardTitle>Connection</CardTitle><CardDescription>OAuth and configured root-folder read test.</CardDescription></CardHeader><CardContent>{status?.connection.status === 'CONNECTED' ? <div className="flex items-center gap-2 text-green-700"><CheckCircle2 className="h-5 w-5" /><span>Connected to root folder <strong>{status.connection.rootName}</strong>.</span></div> : <div className="space-y-3"><div className="flex items-start gap-2 text-destructive"><XCircle className="mt-0.5 h-5 w-5 shrink-0" /><span>{status?.connection.message ?? 'Not tested.'}</span></div>{status?.connection.stage ? <div className="rounded-md bg-muted p-3 text-sm"><p><strong>Failed stage:</strong> {status.connection.stage}{status.connection.code ? <> · <code>{status.connection.code}</code></> : null}</p>{status.connection.suggestion ? <p className="mt-1"><strong>Recommended action:</strong> {status.connection.suggestion}</p> : null}</div> : null}</div>}</CardContent></Card>
