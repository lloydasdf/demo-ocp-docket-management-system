...
type Status = { environment: EnvironmentRow[]; connection: { status: string; account?: { emailAddress: string; displayName: string | null }; rootName?: string; maskedRootId?: string; rootContents?: DriveItem[]; message?: string; stage?: string; code?: string; description?: string; suggestion?: string } };
...
export default function GoogleDriveConfigurationPage() {
  ...
  async function listRootContents() {
    ...
    try { setStatus(await developerRequest('?listRoot=1') as Status); }
      ...
      <div className="flex min-h-screen bg-background"><Sidebar />
        <main className="min-w-0 flex-1 overflow-y-auto p-6 lg:p-10">
          <div className="mx-auto max-w-5xl space-y-6">
            ...
            {status?.connection.status === 'CONNECTED' ? <Card><CardHeader><CardTitle>Root folder contents</CardTitle><CardDescription>List up to 100 direct children of the configured root to verify docket-year placement.</CardDescription></CardHeader><CardContent className="space-y-4"><Button variant="outline" onClick={() => void listRootContents()} disabled={Boolean(action)}><RefreshCw className={`mr-2 h-4 w-4 ${action === 'list' ? 'animate-spin' : ''}`} />List root contents</Button>{status.connection.rootContents ? status.connection.rootContents.length ? <Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Type</TableHead></TableRow></TableHeader><TableBody>{status.connection.rootContents.map((item) => <TableRow key={item.id}><TableCell>{item.webViewLink ? <a className="text-primary underline" href={item.webViewLink} target="_blank" rel="noreferrer">{item.name}</a> : item.name}</TableCell><TableCell>{item.mimeType === 'application/vnd.google-apps.folder' ? 'Folder' : item.mimeType || 'File'}</TableCell></TableRow>)}</TableBody></Table> : <p className="text-sm text-muted-foreground">The root folder is empty.</p> : null}</CardContent></Card> : null}
