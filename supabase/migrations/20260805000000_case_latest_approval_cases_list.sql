...
WITH approval_events AS (
  SELECT
    ...
    cet.code AS approval_event_type_code,
    COALESCE(
      CASE WHEN ce.details_jsonb->>'date_approved' ~ '^\d{4}-\d{2}-\d{2}$' THEN (ce.details_jsonb->>'date_approved')::date END,
      CASE WHEN ce.details_jsonb->>'approved_date' ~ '^\d{4}-\d{2}-\d{2}$' THEN (ce.details_jsonb->>'approved_date')::date END,
      CASE WHEN ce.details_jsonb->>'status_date' ~ '^\d{4}-\d{2}-\d{2}$' THEN (ce.details_jsonb->>'status_date')::date END,
      ce.event_date
    ) AS date_approved,
    COALESCE(
  ...
  WHERE cet.code IN ('CASE_STATUS_APPROVED', 'CASE_DECISION_APPROVED')
    AND ce.is_voided IS FALSE
    AND c.is_archived IS FALSE
    AND public.require_any_app_role(ARRAY['DEVELOPER','CHIEF','ADMIN'])
), ranked AS (
  SELECT *, row_number() OVER (
    PARTITION BY case_id
    ORDER BY date_approved DESC NULLS LAST, time_approved DESC NULLS LAST, created_at DESC, case_event_id DESC
  ) AS approval_rank
...
WITH filtered_cases AS (
  ...
  LEFT JOIN public.v_case_latest_approval vcla ON vcla.case_id = c.id
...
ORDER BY
  fc.docket_year ASC,
  fc.docket_type_id ASC,
  fc.docket_month_code ASC NULLS LAST,
  fc.docket_number ASC,
  fc.id ASC;
$$;

REVOKE ALL ON FUNCTION public.export_cases_excel_data(integer, bigint) FROM PUBLIC;
