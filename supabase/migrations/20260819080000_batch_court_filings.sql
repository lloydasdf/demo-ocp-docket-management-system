...
DECLARE
  filing jsonb; event_id bigint; first_event_id bigint; court_filing_id bigint; number_text text; number_order integer;
...
BEGIN
  ...
  FOR filing IN SELECT value FROM jsonb_array_elements(p_filings) item(value) LOOP
    ...
    SELECT id INTO court_filing_id FROM public.case_court_filings WHERE case_event_id=event_id AND is_voided=false;
    number_order := 0;
    FOR number_text IN SELECT value FROM jsonb_array_elements_text(normalized_numbers) LOOP
      ...
      VALUES(p_case_id,court_filing_id,number_text,number_order,p_user_id,p_user_id)
