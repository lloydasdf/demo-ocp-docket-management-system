...
SELECT
  ...
  CASE WHEN o.id IS NULL THEN NULL::jsonb ELSE jsonb_build_object('id', o.id, 'organization_name', o.organization_name, 'organization_type', o.organization_type, 'registration_number', o.registration_number, 'tax_identification_number', o.tax_identification_number, 'contact_person', o.contact_person, 'contact_number', o.contact_number, 'email', o.email, 'organization_aliases', COALESCE(oa.organization_aliases, '[]'::jsonb)) END AS organizations
...
AS $$
DECLARE
  v_auth_uid uuid := auth.uid(); v_user_id bigint; v_case_id bigint; v_item jsonb; v_sub jsonb;
  ...
  v_region_code text := nullif(btrim(p_payload->>'regionCode'), ''); v_month_code text; v_docket_number int; v_display text; v_dt_prefix text;
  v_person_id bigint; v_org_id bigint; v_name text; v_cp_id bigint; v_address_id bigint; v_violation_id bigint; v_received_event_type_id bigint; v_raffled_event_type_id bigint; v_event_id bigint; v_assignment_id bigint;
  v_assigned_prosecutor_id bigint := CASE WHEN COALESCE((p_payload->>'caseAlsoRaffled')::boolean,false) THEN nullif(p_payload->>'assignedProsecutorId','')::bigint ELSE NULL END; v_assignment_date date := COALESCE(nullif(p_payload->>'assignmentDate','')::date, v_date_received);
...
BEGIN
  ...
  IF v_docket_type_id IS NULL OR v_docket_year IS NULL OR v_date_received IS NULL OR v_initial_status_id IS NULL THEN RAISE EXCEPTION 'Missing required case fields'; END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM public.case_statuses WHERE id = v_initial_status_id) THEN RAISE EXCEPTION 'Invalid initial status id'; END IF;
  ...
  v_month_code := upper(to_char(v_date_received, 'MON')); v_display := concat_ws('-', v_region_code, v_dt_prefix, right(v_docket_year::text,2)||v_month_code, lpad(v_docket_number::text,6,'0'));
  INSERT INTO public.cases(docket_type_id,docket_year,docket_number,date_received,created_by_user_id,updated_by_user_id,is_archived,region_code,docket_month_code) VALUES (v_docket_type_id,v_docket_year,v_docket_number,v_date_received,v_user_id,v_user_id,false,v_region_code,v_month_code) RETURNING id INTO v_case_id;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_payload->'participants') LOOP
    ...
    IF nullif(v_item->>'existingOrganizationId','') IS NOT NULL OR v_item ? 'newOrganization' THEN
      ...
      ELSE v_name := nullif(btrim(v_item#>>'{newOrganization,organizationName}'),''); IF v_name IS NULL THEN RAISE EXCEPTION 'Organization name is required'; END IF; INSERT INTO public.organizations(organization_name,organization_type,registration_number,tax_identification_number,contact_person,contact_number,email,created_by_user_id,updated_by_user_id) VALUES (v_name,COALESCE(nullif(btrim(v_item#>>'{newOrganization,organizationType}'),''),'ORGANIZATION'),nullif(btrim(v_item#>>'{newOrganization,registrationNumber}'),''),nullif(btrim(v_item#>>'{newOrganization,taxIdentificationNumber}'),''),nullif(btrim(v_item#>>'{newOrganization,contactPerson}'),''),nullif(btrim(v_item#>>'{newOrganization,contactNumber}'),''),nullif(btrim(v_item#>>'{newOrganization,email}'),''),v_user_id,v_user_id) RETURNING id INTO v_org_id; END IF;
      FOR v_sub IN SELECT * FROM jsonb_array_elements(COALESCE(v_item->'aliases','[]'::jsonb)) LOOP IF nullif(btrim(v_sub->>'aliasName'),'') IS NOT NULL THEN INSERT INTO public.organization_aliases(organization_id,alias_name,source) VALUES (v_org_id,btrim(v_sub->>'aliasName'),'MANUAL_ENTRY'); END IF; END LOOP;
    ELSE
      ...
      FOR v_sub IN SELECT * FROM jsonb_array_elements(COALESCE(v_item->'aliases','[]'::jsonb)) LOOP IF nullif(btrim(v_sub->>'aliasName'),'') IS NOT NULL THEN INSERT INTO public.person_aliases(person_id,alias_name,alias_type,source) VALUES (v_person_id,btrim(v_sub->>'aliasName'),COALESCE(nullif(btrim(v_sub->>'aliasType'),''),'AKA'),'MANUAL_ENTRY'); END IF; END LOOP;
      FOR v_sub IN SELECT * FROM jsonb_array_elements(COALESCE(v_item->'addresses','[]'::jsonb)) LOOP
        ...
        INSERT INTO public.person_addresses(person_id,address_id,address_type_id,is_primary,remarks) VALUES (v_person_id,v_address_id,(v_sub->>'addressTypeId')::bigint,COALESCE((v_sub->>'isPrimary')::boolean,false),nullif(btrim(v_sub->>'remarks'),''));
      END LOOP;
    END IF;
  ...
  INSERT INTO public.case_events(case_id,event_type_id,event_date,event_order,title,description,status_id,details_jsonb,source,source_table,created_by_user_id,updated_by_user_id) VALUES (v_case_id,v_received_event_type_id,v_date_received,1,'Case received','Initial status set during manual docket creation',v_initial_status_id,jsonb_build_object('docketDisplayNumber',v_display),'MANUAL_ENTRY','case_status_history',v_user_id,v_user_id) RETURNING id INTO v_event_id;
  INSERT INTO public.case_status_history(case_id,from_status_id,to_status_id,changed_by_user_id,changed_at,status_date,remarks,case_event_id) VALUES (v_case_id,NULL,v_initial_status_id,v_user_id,now(),v_date_received,'Initial status set during manual docket creation',v_event_id);
  IF v_assigned_prosecutor_id IS NOT NULL THEN INSERT INTO public.case_assignments(case_id,prosecutor_id,assigned_by_user_id,assigned_at,remarks) VALUES (v_case_id,v_assigned_prosecutor_id,v_user_id,v_assignment_date::timestamp with time zone,'Assigned during manual docket creation') RETURNING id INTO v_assignment_id; INSERT INTO public.case_events(case_id,event_type_id,event_date,event_order,title,description,prosecutor_id,details_jsonb,source,source_table,source_id,created_by_user_id,updated_by_user_id) VALUES (v_case_id,v_raffled_event_type_id,v_assignment_date,2,'Case raffled','Assigned during manual docket creation',v_assigned_prosecutor_id,jsonb_build_object('docketDisplayNumber',v_display),'MANUAL_ENTRY','case_assignments',v_assignment_id,v_user_id,v_user_id); END IF;
