-- A status confirmation modal may be opened immediately after a workflow event.
-- Treat saving its unchanged status/stage selection as a no-op: no timeline,
-- normalized manual update, history, current-state mutation, or audit record.
...
BEGIN
  ...
  IF v_prev_status_id IS NOT DISTINCT FROM p_case_status_id
     AND v_prev_stage_id IS NOT DISTINCT FROM p_case_stage_id THEN
    RETURN 0;
  END IF;
...
COMMENT ON FUNCTION public.record_case_status_updated_event(bigint,bigint,bigint,date,text,text,bigint) IS 'Records a manual CASE_STATUS_UPDATED workflow only when the selected status or stage differs from the stored current state; unchanged selections return 0 without events or audits.';
