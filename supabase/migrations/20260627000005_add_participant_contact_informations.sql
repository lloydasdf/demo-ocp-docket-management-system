CREATE TABLE IF NOT EXISTS public.contact_informations (
  ...
  CONSTRAINT contact_informations_value_not_blank CHECK (btrim(contact_value) <> '')
);

CREATE TABLE IF NOT EXISTS public.participant_contact_informations (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  case_participant_id bigint NOT NULL REFERENCES public.case_participants(id) ON DELETE CASCADE,
  contact_information_id bigint NOT NULL REFERENCES public.contact_informations(id) ON DELETE CASCADE,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT participant_contact_informations_unique UNIQUE (case_participant_id, contact_information_id)
);

CREATE INDEX IF NOT EXISTS contact_informations_contact_type_idx ON public.contact_informations(contact_type);
CREATE INDEX IF NOT EXISTS participant_contact_informations_case_participant_id_idx ON public.participant_contact_informations(case_participant_id);
CREATE INDEX IF NOT EXISTS participant_contact_informations_contact_information_id_idx ON public.participant_contact_informations(contact_information_id);

ALTER TABLE public.contact_informations DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.participant_contact_informations DISABLE ROW LEVEL SECURITY;

GRANT SELECT, INSERT, UPDATE, DELETE ON public.contact_informations TO authenticated, service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.participant_contact_informations TO authenticated, service_role;
GRANT USAGE, SELECT ON SEQUENCE public.contact_informations_id_seq TO authenticated, service_role;
GRANT USAGE, SELECT ON SEQUENCE public.participant_contact_informations_id_seq TO authenticated, service_role;
...
SELECT
  ...
  CASE WHEN cpa.id IS NULL THEN NULL::jsonb ELSE jsonb_build_object('age_text', cpa.age_text, 'age_years', cpa.age_years, 'gender_text', cpa.gender_text, 'gender_normalized', cpa.gender_normalized, 'is_minor_at_case', cpa.is_minor_at_case, 'is_senior_at_case', cpa.is_senior_at_case, 'is_pwd_at_case', cpa.is_pwd_at_case) END AS case_participant_attributes,
  ...
  CASE WHEN o.id IS NULL THEN NULL::jsonb ELSE jsonb_build_object('id', o.id, 'organization_name', o.organization_name, 'contact_person', o.contact_person, 'contact_number', o.contact_number, 'email', o.email, 'details_jsonb', o.details_jsonb, 'organization_aliases', COALESCE(oa.organization_aliases, '[]'::jsonb), 'organization_addresses', COALESCE(org_addr.organization_addresses, '[]'::jsonb)) END AS organizations
...
LEFT JOIN LATERAL (SELECT jsonb_agg(jsonb_build_object('id', ci.id, 'participant_contact_information_id', pci.id, 'contact_type', ci.contact_type, 'contact_value', ci.contact_value, 'label', ci.label, 'is_primary', ci.is_primary, 'remarks', ci.remarks) ORDER BY ci.is_primary DESC NULLS LAST, ci.contact_type, ci.id) AS contact_informations FROM public.participant_contact_informations pci JOIN public.contact_informations ci ON ci.id = pci.contact_information_id WHERE pci.case_participant_id = cp.id) pci ON true
LEFT JOIN LATERAL (SELECT jsonb_agg(jsonb_build_object('id', a.id, 'alias_name', a.alias_name, 'is_active', a.is_active) ORDER BY a.is_active DESC NULLS LAST, a.alias_name) AS person_aliases FROM public.person_aliases a WHERE a.person_id = p.id AND a.is_active IS TRUE) pal ON true
LEFT JOIN LATERAL (SELECT jsonb_agg(jsonb_build_object('id', paddr.id, 'address_id', paddr.address_id, 'address_type_id', paddr.address_type_id, 'is_primary', paddr.is_primary, 'remarks', paddr.remarks, 'addresses', CASE WHEN a.id IS NULL THEN NULL::jsonb ELSE jsonb_build_object('id', a.id, 'barangay', a.barangay, 'city', a.city, 'country', a.country, 'line1', a.line1, 'line2', a.line2, 'province', a.province, 'region', a.region, 'zip_code', a.zip_code) END) ORDER BY paddr.is_primary DESC NULLS LAST, paddr.id) AS person_addresses FROM public.person_addresses paddr LEFT JOIN public.addresses a ON a.id = paddr.address_id WHERE paddr.person_id = p.id) pa ON true
...
LEFT JOIN LATERAL (SELECT jsonb_agg(jsonb_build_object('id', oaddr.id, 'address_id', oaddr.address_id, 'address_type_id', oaddr.address_type_id, 'is_primary', oaddr.is_primary, 'remarks', oaddr.remarks, 'addresses', CASE WHEN a.id IS NULL THEN NULL::jsonb ELSE jsonb_build_object('id', a.id, 'barangay', a.barangay, 'city', a.city, 'country', a.country, 'line1', a.line1, 'line2', a.line2, 'province', a.province, 'region', a.region, 'zip_code', a.zip_code) END) ORDER BY oaddr.is_primary DESC NULLS LAST, oaddr.id) AS organization_addresses FROM public.organization_addresses oaddr LEFT JOIN public.addresses a ON a.id = oaddr.address_id WHERE oaddr.organization_id = o.id) org_addr ON true;
...
-- Replace RPC with participant contact-information support.
CREATE OR REPLACE FUNCTION public.create_new_docket_entry(p_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
DECLARE
  v_auth_uid uuid := auth.uid(); v_user_id bigint; v_case_id bigint; v_item jsonb; v_sub jsonb; v_contact_id bigint;
