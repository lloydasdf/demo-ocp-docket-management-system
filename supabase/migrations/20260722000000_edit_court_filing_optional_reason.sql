...
BEGIN
  ...
  ELSE
    ...
    UPDATE public.case_events SET event_date=p_date_filed,event_time=p_time_filed,description=v_remarks,details_jsonb=coalesce(details_jsonb,'{}'::jsonb) || jsonb_build_object('court_id',p_court_id,'court',v_court_name,'court_name',v_court_name,'court_branch',nullif(btrim(coalesce(p_court_branch,'')),''),'charge_filed',v_charge,'date_filed',p_date_filed,'time_filed',p_time_filed,'criminal_case_numbers',numbers,'court_statuses',statuses,'information_count',p_information_count,'additional_details',details,'remarks',v_remarks),updated_by_user_id=p_user_id,updated_at=now() WHERE id=e.id;
