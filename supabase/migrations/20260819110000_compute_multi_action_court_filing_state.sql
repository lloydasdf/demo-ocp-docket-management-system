-- Count every approval action attached to a logical Court Filing when computing state.

-- Extend automatic state computation to standalone filing workflows.
-- Standalone decision approvals have no case_resolution_id, and standalone
-- Court Filings have no case_resolution_approval_action_id. Both remain active
-- workflow facts and must participate in the same state rules as linked rows.
...
DECLARE
  ...
  v_standalone_filing_count integer;
...
BEGIN
    ...
    count(*) FILTER (
      WHERE aa.decision_code = 'FOR_FILING'
        AND (EXISTS (
          ...
          SELECT 1 FROM public.case_court_filing_actions cfa
          JOIN public.case_court_filings linked_cf ON linked_cf.id = cfa.court_filing_id AND linked_cf.is_voided = false
          WHERE cfa.case_resolution_approval_action_id = aa.id
        ))
    ),
    count(*) FILTER (
      WHERE aa.decision_code = 'FOR_FILING'
  ...
  JOIN public.case_resolution_approvals a
   ...
   AND a.is_voided = false
  LEFT JOIN public.case_resolutions cr
    ON cr.id = a.case_resolution_id
  WHERE aa.case_id = p_case_id
    AND (a.case_resolution_id IS NULL OR cr.is_voided = false);

  SELECT count(*)
  INTO v_standalone_filing_count
  ...
  WHERE cf.case_id = p_case_id
    AND cf.is_voided = false
    AND cf.case_resolution_approval_action_id IS NULL;

  v_filed_for_filing_count := COALESCE(v_filed_for_filing_count, 0)
    + COALESCE(v_standalone_filing_count, 0);
...
COMMENT ON FUNCTION public.compute_current_case_state(bigint) IS 'Computes chronological current state for linked and standalone Case Decision Approved and Court Filing workflows, plus explicit status-producing events.';
