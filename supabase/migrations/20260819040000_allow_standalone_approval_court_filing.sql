-- Permit Court Filing to consume a standalone approved FOR_FILING action.
-- A standalone Case Decision Approved event intentionally has no case resolution.
...
BEGIN
  ...
  IF p_case_resolution_approval_action_id IS NOT NULL THEN
    ...
    WHERE aa.id = p_case_resolution_approval_action_id AND aa.case_id = p_case_id
      AND aa.decision_code = 'FOR_FILING' AND a.is_voided = false
    ...
    IF EXISTS (
      ...
      WHERE cf.case_resolution_approval_action_id = p_case_resolution_approval_action_id
        AND cf.is_voided = false
    ) THEN
      RAISE EXCEPTION 'This approved filing decision already has an active Court Filing.';
    END IF;
  END IF;
