...
SELECT
  ...
  CASE WHEN o.id IS NULL THEN NULL::jsonb ELSE jsonb_build_object('id', o.id, 'organization_name', o.organization_name, 'contact_person', o.contact_person, 'contact_number', o.contact_number, 'email', o.email, 'details_jsonb', o.details_jsonb, 'organization_aliases', COALESCE(oa.organization_aliases, '[]'::jsonb)) END AS organizations
...
BEGIN
  ...
  INSERT INTO public.case_events(case_id,event_type_id,event_date,event_order,title,description,status_id,details_jsonb,source,source_table,created_by_user_id,updated_by_user_id) VALUES (v_case_id,v_received_event_type_id,v_date_received,1,'Case received',v_case_received_description,v_initial_status_id,jsonb_build_object('docketDisplayNumber',v_display),'MANUAL_ENTRY','case_status_history',v_user_id,v_user_id) RETURNING id INTO v_event_id;
  INSERT INTO public.case_status_history(case_id,from_status_id,to_status_id,changed_by_user_id,changed_at,status_date,remarks,case_event_id) VALUES (v_case_id,NULL,v_initial_status_id,v_user_id,now(),v_date_received,v_case_received_description,v_event_id) RETURNING id INTO v_status_history_id;
