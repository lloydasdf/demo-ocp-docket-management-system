-- Permit standalone Case Decision Approved events while retaining the complete approval workflow.
-- Resolution-specific validations apply only when an existing resolution is linked.
BEGIN;
...
COMMENT ON FUNCTION public.record_case_decision_approved_event(bigint, bigint, bigint, date, time without time zone, jsonb, text, bigint) IS 'Records CASE_DECISION_APPROVED with an optional linked case resolution; resolution ownership, voided-resolution, duplicate-approval, and source-resolution-action validations apply only when a resolution is provided.';
