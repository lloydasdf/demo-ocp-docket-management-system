...
COMMENT ON FUNCTION public.record_case_decision_approved_event(bigint, bigint, bigint, date, time without time zone, jsonb, text, bigint) IS 'Records CASE_DECISION_APPROVED using split Case Status / Case Stage: approval action outcome remains on the approval row while case_events, case_private_details, case_status_history, and case_stage_history use public.compute_current_case_state().';

-- Verification SQL/comments (run in a seeded transaction and roll back):
-- 1. Approving a FOR_FILING resolution with no other pending work: SELECT status.code, stage.code from case_private_details joins after public.record_case_decision_approved_event(... FOR_FILING action ...) -> PENDING / FOR_FILING.
-- 2. Approving a DISMISSAL resolution with no other pending work -> DISMISSED / DISMISSED.
-- 3. Mixed approved filing and dismissal outcomes across active approvals, with every filing action filed, -> MIXED_RESULT / MIXED_RESULT.
-- 4. Another active resolution awaiting approval at approval-record time -> PENDING / RESO_FOR_APPROVAL when no filing has been recorded yet.
-- 5. Filed case with another approved filing still waiting to be filed -> PENDING / FILED_OTHER_INFO_FOR_FILING.
-- 6. Filed case with another resolution awaiting approval -> PENDING / FILED_OTHER_RESO_FOR_APPROVAL.
-- 7. Voiding an approval that restores an unapproved resolution through public.void_case_event(approval_event_id, ...) marks only the approval voided, preserves approval actions/history/audit, invokes public.apply_case_state_recompute(...) once, and recomputes -> PENDING / RESO_FOR_APPROVAL when that unfinished resolution is the highest-priority active workflow.
