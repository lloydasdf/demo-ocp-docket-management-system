ALTER TABLE public.audit_logs
  ADD COLUMN IF NOT EXISTS case_id bigint REFERENCES public.cases(id),
  ADD COLUMN IF NOT EXISTS summary text,
  ADD COLUMN IF NOT EXISTS metadata jsonb;

CREATE INDEX IF NOT EXISTS idx_audit_logs_case_id_time ON public.audit_logs(case_id, created_at DESC);

-- Replace RPC with audit logging after successful docket creation.
