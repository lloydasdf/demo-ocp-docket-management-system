...
LEFT JOIN LATERAL (
  SELECT jsonb_agg(jsonb_build_object('alias_name', a.alias_name, 'alias_type', a.alias_type, 'is_active', a.is_active) ORDER BY a.is_active DESC NULLS LAST, a.alias_name) AS person_aliases
...
LEFT JOIN LATERAL (
  SELECT jsonb_agg(jsonb_build_object(
    'id', paddr.id,
    'address_id', addr.id,
    'address_type_id', paddr.address_type_id,
    'is_primary', paddr.is_primary,
    'remarks', paddr.remarks,
    'addresses', CASE WHEN addr.id IS NULL THEN NULL::jsonb ELSE jsonb_build_object(
      'id', addr.id,
      'address_id', addr.id,
  ...
  ) ORDER BY paddr.is_primary DESC NULLS LAST, paddr.id) AS person_addresses
...
LEFT JOIN LATERAL (
  SELECT jsonb_agg(a.alias_name ORDER BY a.is_active DESC NULLS LAST, a.alias_name) AS aliases
...
REVOKE ALL ON public.v_organization_search FROM anon;
GRANT SELECT ON public.v_organization_search TO authenticated, service_role;
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_payload->'participants') LOOP
    ...
    IF v_person_id IS NOT NULL AND v_item ? 'attributes' AND v_item->'attributes' <> 'null'::jsonb THEN
