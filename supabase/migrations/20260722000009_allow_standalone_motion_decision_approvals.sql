...
BEGIN
  ...
  FROM (
    ...
    WHERE a.case_id=p_case_id AND a.is_voided=false AND a.updates_case_status=true
      AND a.selected_case_status_id IS NOT NULL AND a.selected_case_stage_id IS NOT NULL
    UNION ALL
...
COMMENT ON FUNCTION public.compute_current_case_state(bigint) IS 'Manual Case Status Updated, standalone or linked Motion Decision Approved, Petition Update, and Custom Event explicit statuses share deterministic explicit-state recompute; active Custom Events with updates_case_status=true participate in void recomputation.';
