...
BEGIN
  ...
  IF v_action IN ('add', 'edit') THEN
    IF v_violation_id IS NULL THEN
      ...
      WHERE canonical_title = v_canonical_title
         OR regexp_replace(lower(btrim(title)), '\s+', ' ', 'g') = v_canonical_title
         OR regexp_replace(lower(btrim(short_label)), '\s+', ' ', 'g') = v_canonical_title
         OR regexp_replace(lower(btrim(reference_code)), '\s+', ' ', 'g') = v_canonical_title
      ...
      ELSE
        UPDATE public.violations SET is_active = true WHERE id = v_violation_id;
      END IF;
    ...
    IF v_action = 'add' THEN
      ...
      ON CONFLICT (case_id, violation_id) DO UPDATE
